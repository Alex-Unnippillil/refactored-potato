...
def main():
    ...
    with sync_playwright() as p:
        ...
        for name,route,width,height in [('workspace-desktop','/',1440,1080),('workspace-mobile','/',390,1100),('operations-desktop','/operations',1440,1080),('operations-mobile','/operations',390,1100)]:
    ...
    (output/'capture.json').write_text(json.dumps({'mode':'fictional demo','transport':'real HTTP; no browser mocks','device_scale_factor':2,'source_sha256':digest,'images':4},indent=2)+'\n')
    print('Captured four real-HTTP demo screenshots; no browser errors or horizontal overflow.')
