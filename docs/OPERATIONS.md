...
For the initial connector-based publication, `.github/workflows/source-delivery.yml` accepts a checksum-verified, source-only compressed bundle committed by an authorized repository writer. It rejects credentials and workflow paths, validates the full extracted app against real PostgreSQL and three browsers, then publishes normal source files with a fast-forward Git push. It never force-pushes. The transport bundle is removed on success. This is a delivery aid, not an application runtime dependency. Normal development edits the source files and uses the CI workflow directly.
