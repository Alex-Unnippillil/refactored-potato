...
- Schema migration, operator CLI, regression evaluation, API/security tests, real PostgreSQL integration tests, browser tests, deployment configuration and an operations guide.

## Run the demo

Use Python 3.12 or newer. No frontend build step or Node dependencies are required.
...
Do not treat a successful `/api/health` response as database/provider readiness. That endpoint is liveness only. In live mode, run an authenticated search, inspect evidence and source freshness, and verify the expected commit in Vercel's deployment details. See [operations](docs/OPERATIONS.md) and [security](docs/SECURITY.md).
